...
export const InputVariant = ({placeholder, value, setValue, type, name, label, className}) => {
    return (
        <div className='display_inline'>
            {label && <div>{label}</div>}
            <input name={name} type={type? type:'button'} value={value} className={`input small_input margin_around ${className}`} placeholder={placeholder} onChange={e=>setValue(e.target.value)} />
        </div>
    )
}
...
export const TextArea = ({placeholder, onChange, value, label}) => {
    return (
        <div className='text_area_container'>
            {label && <div>{label}</div>}
            <textarea value={value} className='text_area' placeholder={placeholder} onChange={onChange}/>
        </div>
    )
}
